  ...
  inputs:{
    ...
    play(element){
      ...
      if(!!game.turn){
        player='x'
        game.turn=0
        img.setAttribute('src',Object.values(game.xo)[1][index])
      }else{
        player='o'
        game.turn=1
        img.setAttribute('src',Object.values(game.xo)[0][index])
      }
...
document.getElementById('choisesSection1').onclick=(e)=>{
  if(e.target.localName==='button'){
    if(e.target.textContent==='X'){game.turn=1}
    if(e.target.textContent==='O'){game.turn=0}
    document.querySelectorAll('.choise')[0].remove()
    document.getElementById('chosentemplate').style.display='block'
  }
}
document.getElementById('choisesSection2').onclick=(e)=>{
  game.level.grids=parseInt(document.querySelector('.grids')
  [document.querySelector('.grids').selectedIndex].textContent)
  game.level.required=parseInt(document.querySelector('.required')[document.querySelector('.required').selectedIndex].textContent)
  if(e.target.localName==='button' && e.target.textContent==='Start!'){
