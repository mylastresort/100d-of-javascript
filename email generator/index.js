  ...
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      switch (action) {
        ...
        case 'checkAll':
          listId = new Array();
          document.querySelectorAll('tr').forEach(_ => listId.push(_.id));
          //showing the messages on the window
          data.forEach((element) => {
            if (!listId.some(_ => _ == element.id)) {
              ...
              let item = document.createElement('th');
              item.setAttribute('id', 'items')
              item.innerHTML = '<i onclick="expand(this)" id="expand" style="font-size:100%; opacity:0.7; cursor:pointer;" class="fas fa-angle-right"></i>';
              message.appendChild(item);
...
function expand(e) {
  if(document.querySelector('.expand').style.width !== '25vw') {
    ...
    document.querySelector('.messages-section').style.marginLeft = '20%';
    document.querySelector('.expand').style.width = '0vw';
